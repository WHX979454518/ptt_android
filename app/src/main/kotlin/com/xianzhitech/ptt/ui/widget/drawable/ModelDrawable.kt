...
private class ModelDrawable constructor(private val context: Context,
    ...
    init {
        ...
        when (model) {
            is User -> drawable = model.createAvatarDrawable()
            is Group -> {
        ...
        override fun onNext(t: List<User>) {
            modelDrawableRef.get()?.apply {
                ...
                if (t.size <= 2 && currentUserId != null && excludeCurrentUser) {
                    drawable = t.firstOrNull { it.id != currentUserId }?.createDrawable()
                } else {
                    drawable = MultiDrawable(context, t.map { it.createDrawable() })
                }
            } ?: unsubscribe()
        }
    }
...
@JvmOverloads
fun NamedModel.createDrawable(maxMemberDisplayCount: Int = Constants.MAX_MEMBER_ICON_DISPLAY_COUNT): Drawable {
    return ModelDrawable(App.instance, this, maxMemberDisplayCount)
}